...
async function runScan() {
    ...
    for (const stock of holdings) {
        ...
        const news = await (0, newsFetcher_1.fetchNewsForCompany)(stock.name, stock.symbol);
        if (!news.length) {
        ...
        const report = await (0, analyzer_1.analyzeHoldings)(stock, news);
    ...
    if (highRiskReports.length > 0) {
        // 只有真正有高风险才发邮件
        console.log(`\n🚨 发送预警邮件...`);
        await (0, notifier_1.sendEmailAlert)(highRiskReports);
    }
